...
present a hardware-focused analog predictive-coding architecture that
trains in place under three invariants --- signal paths are one way
(\emph{forwards-only}), no per-device calibration (\emph{robust}), and
no global clock (\emph{asynchronous}) --- and show they cost essentially
no accuracy. This is accomplished by a separated weights and error
design, computing each weight matrix's transpose on the chip that owns
it and a sigma--delta weight update via a \emph{leaky-jug} error
accumulator and a threshold comparator, whose cumulative quantisation
error is bounded independently of the number of updates (the first-order
sigma--delta property). In this design a per-update step far below one
weight quantisation step drives a coarse analog cell with no per-synapse
digital accumulator, which means the comparator may be wrong at a
substantial rate without corrupting learning, and device mismatch
appears only as a benign per-synapse spread of the learning rate. On
EMNIST letters with a 48-chip topology the forwards-only rule reaches
\(82.50\%\) against a full-backpropagation ceiling of \(82.85\%\) on
identical topology, and a bit-faithful hardware model stays within the
run-to-run noise band of its floating-point ceiling. All results are
pre-silicon.
\end{abstract}
...
place as a transconductance --- a device whose output current is
proportional to its input voltage --- with summation by Kirchhoff's
current law, then the weights never move and the dominant cost is
removed by construction. Part I of this study \cite{dobney2026analog}
demonstrated such a substrate for predictive-coding networks: a compact
analog cell that stores a weight as a capacitor voltage and learns
...
This paper asks whether the same substrate can be driven to task
accuracy on real data by \emph{supervised} learning, and what the
learning hardware would have to look like. Our stance is hardware-led:
we ask what a realistic, natural hardware system can actually do, and
then search for a learning rule that fits within those hardware
constraints, rather than designing an idealised algorithm and asking the
...
with the forward pass. In digital hardware this is routine and easily
coded. In an analog in-memory substrate it is not: an analog
transconductor is a one-way device, and current cannot simply be driven
backwards through it. The alternatives are a second, reverse analog
datapath --- doubling the analog design problem and requiring the two
directions to be held in alignment --- or a digital copy of every weight
held wherever the transpose is computed, which restores the weight
traffic that motivated the substrate. Either way, the resulting system
needs a global schedule to keep forward and backward passes coherent,
which is the property that makes large analog neuromorphic fabrics
fragile and hard to grow.

This barrier is not unique to predictive coding; it is a difficulty for
the whole analog-learning field, approached from several directions.
Analog in-memory training on non-volatile memory (NVM) has reached
...
has also reported synthesisable register-transfer-level (RTL)
realisations of PC networks \cite{oh2026synthesizable}. What has been
missing is an analog PC design that trains in place \emph{without}
reintroducing the backward pass, the host, or the calibration step.

This paper reports such an architecture, together with its validation.
We adopt three constraints as design invariants rather than objectives,
and do not relax them:
...
Two further principles are design commitments rather than hard
invariants: the design is \textbf{modular} --- the network topology is a
soft, reconfigurable property of the inter-chip router rather than fixed
wiring --- and \textbf{hardware-feasible} --- every element is chosen to
be synthesisable or fabricable in an open \(130\) nm process (Sky130A).
We began by testing the separated-weights architecture with contrastive
Hebbian learning (CHL), the canonical predictive-coding rule, but found
it too fragile under these constraints (Sec. 2); the rule reported here
...
\item
  ...
  weight update is a local transaction (Sec. 4).
\item
  ...
  and reduces the router to summation and division (Secs. 4, 5).
\item
  ...
  far below one weight code step can drive a coarse analog cell with no
...
For readers coming from either side of this work, the recurring terms
are collected here.

\begin{itemize}
\tightlist
\item
  \textbf{PC} --- predictive coding. \textbf{CHL} --- contrastive
  Hebbian learning, the canonical PC weight rule.
  \textbf{\(\bm W\&\bm E\)} --- the design's \emph{separated} weight
  store \(\bm W\) and error store \(\bm E\).
\item
  \textbf{\(\bm\delta\)} --- a transported error signal;
  \textbf{\(\bm W^{\!\top}\)} --- the transpose (column-wise read) of a
  weight matrix, needed to route error to the previous layer.
\item
  \textbf{Weight code / quantisation step} --- the analog weight is set
  by an 8-bit digital code; one code step (\(1/64\) of the weight range
  here) is the smallest weight change, the analog analogue of a
  \emph{least-significant bit} (LSB).
\item
  \textbf{Leaky jug} --- the per-synapse error accumulator: a capacitor
  that integrates error, leaks slowly, and ``fires'' one weight code
  step when it crosses a threshold. Its formal reading is a first-order
  \textbf{sigma--delta} (\(\Sigma\Delta\)) modulator.
\item
  \textbf{Hardware acronyms} (Sec. 4 onward): \textbf{OTA}, operational
  transconductance amplifier (the analog multiplier cell); \textbf{MAC},
  multiply--accumulate; \textbf{SRAM}, static memory holding the weight
  codes; \textbf{DAC/ADC}, digital-to-analog / analog-to-digital
  converter; \textbf{SAR}, successive-approximation register (the ADC
  type); \textbf{AGC}, automatic gain control; \textbf{RTL},
  register-transfer level (a synthesisable digital description);
  \textbf{SPICE}, the analog circuit simulator; \textbf{NVM},
  non-volatile memory.
\end{itemize}
...
activity \cite{whittington2017approximation}. We began our research by
testing the \(\bm W\&\bm E\) architecture with contrastive Hebbian
learning, but the scheme we describe instead learns by approximating the
backpropagation gradient, because of the difficulties --- set out next
--- of making CHL work as a hardware solution.

\hypertarget{sec:pc-challenges}{%
\subsection{Four obstacles to an analog realisation of
CHL}\label{sec:pc-challenges}}

Canonical predictive coding using CHL, and the SNN tradition it is
adjacent to, present four obstacles to an analog implementation. These
are the reasons the search moved away from CHL; each is answered later
by a specific element of the design.
...
Analog storage is sensitive to the size of each update. An effective
...
therefore \(\approx 0.019\) of one weight code step: rounded to the
cell's resolution it is exactly zero, roughly fifty times in succession.
Writing a correct gradient to a coarse analog cell naively does not slow
learning down; it stops it altogether. Updates can be passed through a
high-precision digital \emph{master weight} per synapse that accumulates
sub-step updates and is periodically written down, but our hardware
...
The design answers C1 by relocating the transpose onto the chip that
owns the weights \(\bm W\) (Sec. 4), C2 by abandoning relaxation in
favour of a single forward evaluation per sample (Sec. 3), C3 by
replacing the digital master weight with a sigma--delta accumulator
\(\bm E\) (Sec. 3), and C4 by making the message class a configurable
property of the interconnect (Sec. 5).

\hypertarget{sec:theory}{%
\section{The learning rule}\label{sec:theory}}

This section states the rule as an algorithm, independently of how it is
built; the hardware realisation follows in Sec. 4. The rule uses
ordinary backpropagation's credit assignment, evaluated \emph{once} per
sample and written to the weights through a threshold-accumulator
instead of an arithmetic add. The approach of backprop with the
threshold-accumulator was chosen to meet the constraints of a
hardware-first design. Core to this is the separation of weights and the
forwards path, from the `errors' and the error propagation path, with a
leaky-accumulator used as the mechanism for updating the weights in the
learning phase. The same credit assignment as ordinary backpropagation
is computed, but it is delivered through a forwards-only,
hardware-robust design --- a quantised threshold write over small-matrix
networks rather than the massive matrix multiplies of conventional
backpropagation.

Structurally the rule is a loop with two separated stores, and it is
worth seeing the whole of it before the algebra. The forward path
carries a sample through the weights, input
\(\to \bm W_1 \to \bm W_2 \to \dots \to\) read-out, and the read-out
forms a single output error \(\bm\delta\) --- one vector, not a field of
them. That error is then relayed layer by layer, \emph{backward through
the network but forward in signalling}: each layer computes
\(\bm W^{\!\top}\bm\delta\) from the weights it already holds, hands the
result to the layer before it, and drops its own share into a local
error store. The store is a leaky accumulator --- a \emph{jug} --- that
integrates what it receives and, only when it crosses a threshold,
writes a single \(\pm1\) code step into that layer's weights. There is
no relaxation phase and no reverse analog pass: one forward sweep, one
error relay, one conditional weight step per synapse (Fig.~\ref{fig:overview}).
...
block-structured by the chip factoring of Sec. 4: a chip owns a set of
...
per sample --- and is broadcast to scale the update severity (Sec. 3.3);
...
architectural content, developed in Sec. 4, is \emph{where} each factor
is evaluated. The product \(\bm W_\ell^{\!\top}\bm\delta_\ell\) is a
transpose of the layer's own weights; the Jacobian factor
\(\varphi'(\bm a_{\ell-1})\) is a sign test on that layer's forward
activation; and the sum over the parts contributing to a destination is
a plain addition. No unit needs a weight it does not own.

Two operators appear, and both are consequences of quantised,
finite-range signalling rather than of the mathematics. \(\mathcal{Q}\)
is a fixed-range \(b\)-bit quantiser (\(b = 6\) here) --- the finite
resolution of the error path. \(\mathcal{A}_\ell\) is a slow per-layer
automatic gain control (AGC) that tracks the running root-mean-square
(RMS) of the layer's error with an exponential moving average and
rescales it to a fixed target. The AGC is not cosmetic. The transported
error attenuates by \(\approx 0.72\times\) per layer --- the classical
vanishing-gradient factor \cite{bengio1994learning}, set by the weight
norm --- so that under a \emph{fixed-range} quantiser the error
eventually underflows the smallest code and \(\mathcal{Q}\) maps it
...
is a value \(E\) that is never reset. It leaks; incoming error tops it
up; and when it crosses a threshold \(\theta\) the synapse \emph{fires}:
the weight code moves by one step and \(\theta\) is \emph{subtracted}
from the store, retaining the residue (Fig.~\ref{fig:jug}). For one synapse, per
sweep \(t\), \begin{equation}
...
training-specific corollaries the bounded-error property yields; the
modulator itself is not new.
...
comparison decides, the update to \(E\) and the update to \(W\) are the
...
sweep is small, \(|E_T|\) remains bounded by the store's clamp
\(E_{\max}\). Therefore
\begin{equation}
...
weight code --- a bound \emph{independent of \(T\)}. The weight tracks
the accumulated gradient scaled by \(1/\theta\), with an error that does
not grow with the length of training. This is the sigma--delta property,
and it is the mathematical content of the design: rounding each update
discards a fixed fraction of every step (here, all of it); the jug
defers steps but discards none.
...
is the parameter to tune. It is a fixed reference, set once, and is
never discovered at runtime.

\emph{The comparison may be wrong.} Identity \eqref{eq:invariant} does not assume \(s_t\)
is correct. A wrong-signed fire moves the weight the wrong way
\emph{and} pushes the residue further from zero by \(\theta\), which
makes the next fire more likely and of the correct sign; the accumulated
quantity is conserved and the error is repaid. What must be accurate is
the \emph{stored quantity}, not any decision made about it. This inverts
the usual analog design burden: the threshold comparison, normally a
precision operation, becomes the loose one.
...
The store's leak and the optimiser's velocity register are the same
object, so the register is not implemented but inherited. In the
hardware (Sec. 4) the physical leak available is far slower than needed,
so on the firing timescale the store behaves as a pure integrator, which
is the best-performing configuration measured; the requirement is a
loose one-sided bound, not a servo.

\hypertarget{sec:arch}{%
\section{Hardware realisation}\label{sec:arch}}

Having fixed the rule, we now describe how each of its computations is
carried out in silicon. The design is a mixed analog--digital one that
allows flexible topologies: the core compute is analog, done with
capacitance and transconductance, but the weight \emph{values} are held
digitally in SRAM to protect them from decay on a capacitor. We describe
it from the unit that matters most --- the chip and its single weight
memory --- outward to the network.

The commitments a PCN or SNN reader can hold throughout are these. (i)
Representation and error are separate populations (\(\bm W\&\bm E\)).
(ii) Each chip keeps exactly one copy of its weights in SRAM and reads
it three ways --- forward prediction, transpose relay, and error
accumulation. (iii) The inter-chip router only \emph{gathers} (sum and
divide); it holds no weights, so the network topology is a soft,
reconfigurable property of the router rather than fixed wiring. (iv) All
signalling is forward-directed; there is no reverse analog channel and
no counter-propagating pipeline. (v) The update approximates the
backpropagation gradient of Sec. 3, driven by the minimal ``happiness''
scalar that gates severity without carrying per-unit information. (vi)
Every element is chosen to be synthesisable or fabricable in an open
\(130\) nm process (Sky130A).

\hypertarget{the-substrate-and-its-granularity}{%
\subsection{The substrate and its
granularity}\label{the-substrate-and-its-granularity}}

The compute element, inherited from Part I \cite{dobney2026analog}, is
a five-transistor operational transconductance amplifier (OTA): an 8-bit
code held in SRAM (one step per \(1/64\) of the weight range) is
converted to a voltage on a \(200\) fF capacitor, which sets the tail
current of a differential pair and hence the transconductance by which
the cell multiplies its input. Column currents sum on a shared node by
Kirchhoff's current law --- the analog multiply--accumulate (MAC) ---
and an 8-bit successive-approximation-register (SAR) analog-to-digital
converter (ADC) digitises the result. The array unit is a \(16\times16\)
\emph{cell} (\(256\) such multipliers); a \emph{chip} carries four
cells; a \emph{router} mediates all traffic between chips. Routing is
performed at cell granularity and never finer, which keeps the routing
state \(O(\text{cells}^2)\) rather than \(O(\text{synapses}^2)\);
fine-grained mixing is the job of the destination cell's own weight
matrix, not of the network. Because the router, not the wiring, defines
which cell feeds which, the topology is reconfigurable and open to
experiment.
...
weight digital-to-analog converter (DAC) is driven through a fixed
pre-distortion table (Fig.~\ref{fig:chip}) so that stored code maps monotonically to
effective weight. With the measured cell curve and this pre-distortion
in place, accuracy is \(81.46\%\) --- within the run-to-run noise band
of the \(81.96\%\) obtained under an ideal linear cell (Sec. 6) --- so
the bit-faithful model's linear-weight treatment is a consequence of the
pre-distortion, not an assumption.
...
\item
  ...
  multiplies the incoming activation. This is the in-memory forward pass
  and realises \(\bm a_\ell = \bm W_\ell \bm x_{\ell-1}\).
\item
...
\item
  ...
  the locally latched forward activity, forms the per-synapse increment
  \(g_t\) of \eqref{eq:jug}; a threshold crossing writes a single \(\pm1\) step
  back to the SRAM.
\end{enumerate}
...
The analog array exists for \emph{inference}, where the forward MAC sets
the power and area budget; learning is a training-time addition.
Computing \(\bm W^{\!\top}\bm\delta\) digitally --- only during
training, at 6-bit precision, on one engine time-multiplexed across the
four cells --- adds a small, amortised cost that never touches the
inference path, and in exchange keeps the analog array strictly one-way.
The question this paper answers is therefore not \emph{analog versus
digital learning} but whether an analog inference substrate can be made
to learn in place without a reverse analog channel, a host, or a clock.
...
weight traffic, a coherence barrier, and a drift failure mode.
Calculating the transpose on the source chip means chips only need to
pass on errors not a set of weight updates.
...
\hypertarget{sec:jughw}{%
\subsection{The error store in hardware}\label{sec:jughw}}

The rule's update path \eqref{eq:jug}--\eqref{eq:bound} is realised by a leaking capacitor
\(C_e\) and a shared comparator. The mathematics asked for three things
of this circuit, and each turns a normally-tight analog requirement into
a loose one.

\emph{One tight specification, and only one.} The mechanism asks for
analog accuracy in exactly one place: the residue subtraction must
remove a fixed quantity \(\theta\) independent of the capacitor's
voltage, realised as a current-steered charge pulse \(Q = It\). Identity
\eqref{eq:invariant} is the reason it is the \emph{only} tight specification --- because
the stored quantity is conserved across the fire event, everything
downstream of it, the comparator decision included, is permitted to be
loose.

\emph{The comparator is shared and swept, not per-synapse.} A single
comparator walks the array, so a synapse is tested once per sweep and
can fire at most once per sweep. This is a design commitment rather than
an approximation: it is \(256\times\) cheaper per cell, and the sweep
period acts as a refractory interval. A per-synapse comparator would
allow repeated firing on a single crossing; we measure this to be
...
would be the wrong rate limiter, as it destroys the stored quantity that
\eqref{eq:invariant} requires.

\emph{The leak physically realises momentum.} The available leak is
\(\approx 50\,\mu\)V/s on \(100\) fF, i.e.~\(\approx 140\) s per error
code step, against a mean inter-fire interval of tens of milliseconds
--- so on the firing timescale the capacitor is a pure integrator, the
momentum limit \(\lambda \to 1\) of Sec. 3, and the requirement
\(\tau_{\text{leak}}\gg\) inter-fire interval is met by a factor of
\(10^3\).

\emph{The error is kept out of the forward path.} \(C_e\) is compared,
never summed into the MAC. An earlier cell placed a second tail
transistor driven by \(C_e\) so that the array computed
...
validated rather than assumed. Forward activations are delivered
...
all the jug requires is that quantity arriving over time be conserved.
...
physics permit it?), and register-transfer-level (RTL) simulation (does
the digital realisation reproduce the arithmetic?). The behavioural rig
is reported here; the design has additionally been carried to Sky130A
SPICE and to a bit-faithful RTL implementation, whose netlists,
listings, and test benches are in the repository, and we draw on those
levels only where a specific claim depends on them.
...
pre-distortion in place, \(81.46\%\); Sec. 4.1). The move to hardware
...
register, and all per-synapse digital state that a conventional sub-step
...
Table 2 tests the three corollaries of Sec. 3.3 against deliberately
...
Sec. 4.3 introduced one load-bearing assumption: that the forward pass
...
weight code and every residue of \eqref{eq:jug} within \(2\,\mu\)V of the model;
netlists and test benches are in the repository.)
...
\hypertarget{sec:scale}{%
\section{Separability, modularity, and scale}\label{sec:scale}}

One decision underlies every property claimed above, and it is clearest
stated here, once the pieces are on the table. The decision is to
\textbf{separate the weights from the errors} --- to treat the forward
computation and the learning signal as two stores on two paths
(\(\bm W\&\bm E\)), rather than as one object updated in place by a
backward pass coupled to the forward one. The three invariants of the
introduction, and the modularity that lets the fabric grow, are
consequences of that separation rather than independent achievements.

\textbf{Separation makes the neuron self-contained.} Once the error
travels as an ordinary forward-directed message rather than as a current
pushed back through the array, a weight is never needed anywhere but on
the die that owns it. That is what allows a single weight memory to
serve all three of its consumers --- forward prediction, transpose
relay, and error accumulation --- from one copy. There is no master
weight held elsewhere, no second copy to keep coherent, and no quantity
in a weight update that lives off the die.

\textbf{Self-containment makes the tile modular.} A unit that needs
nothing external can be replicated without qualification. The analog
array, its transpose engine, and its jug together form a complete
learning cell; a network is assembled by tiling copies of that cell and
wiring them through the router, not by designing a fresh datapath for
each layer or width. The cell that learns one layer learns any layer,
which is why the same \(16\times16\) tile is the unit of both
computation and growth.

\textbf{Modularity makes the system scalable.} Because every tile
updates locally and asynchronously --- each has on hand every quantity
its update needs, and no step waits on a global phase --- enlarging the
network is a matter of adding tiles and interconnect, not of
re-establishing coherence across a larger fabric. The property that
makes large analog neuromorphic systems fragile, a global schedule
holding forward and backward passes in lockstep, is absent by
construction. What bounds scale is interconnect bandwidth and area, both
of which grow gracefully, rather than a synchronisation cost that
worsens with size.

Read forwards, the chain is \emph{separation} \(\to\)
\emph{self-containment} \(\to\) \emph{modularity} \(\to\) \emph{scale},
and it is why the invariants cost so little accuracy rather than so
much. Forwards-only is what permits the single weight copy; robustness
is what lets a coarse, mismatch-ridden cell be replicated without
per-die calibration; asynchrony is what lets the replicas run without a
shared clock. They are not three separate constraints that happened to
be satisfiable together, but three aspects of the one separation --- and
the accuracy figures of the evaluation are the measurement that paying
for it costs essentially nothing.
