<!-- ============================================================================
EDITABLE MARKDOWN TWIN — main_stage2_v4_postbogacz_1  (reorg of v4, 2026-08-20)

POST-BOGACZ REORGANISATION. Same content, tone, and depth as v4; the changes are
structural + clarity only, addressing R. Bogacz's review:
  * the ALGORITHM (the learning rule, Sec. 3) is now stated BEFORE the hardware
    realisation (Sec. 4), and the two are kept separate (his main point);
  * every acronym/concept is defined on first use + a short Terms box;
  * the abstract no longer opens with undefined jargon (LSB removed).
No new results; the constraint-ledger idea and the new EMNIST numbers are NOT here
(the latter are a Paper-3 point).

Build to PDF:  python3 build_postbogacz.py  then pdflatex/bibtex/pdflatex x2.
The .tex it emits (main_stage2_v4_postbogacz_1.tex) takes equations/figures/preamble
verbatim from main_stage2_v3.tex, so numbered-equation references stay correct.

Edit prose freely here and hand back; I port edits into the .tex and rebuild.
FIGURES are TikZ and cannot round-trip; their captions live in v3.tex / the build
script, so edits to the figure blockquotes below are cosmetic for the .md only.
============================================================================ -->

---
abstract: |
  Analog architectures for predictive-coding networks benefit from in-memory computation which removes the weight-movement cost that dominates digital neural inference, but currently there are no established mechanisms for *supervised training in place* that avoid a backward pass, per-device calibration, or a host in the loop. We present a hardware-focused analog predictive-coding architecture that trains in place under three invariants --- signal paths are one way (*forwards-only*), no per-device calibration (*robust*), and no global clock (*asynchronous*) --- and show they cost essentially no accuracy. This is accomplished by a separated weights and error design, computing each weight matrix's transpose on the chip that owns it and a sigma--delta weight update via a *leaky-jug* error accumulator and a threshold comparator, whose cumulative quantisation error is bounded independently of the number of updates (the first-order sigma--delta property). In this design a per-update step far below one weight quantisation step drives a coarse analog cell with no per-synapse digital accumulator, which means the comparator may be wrong at a substantial rate without corrupting learning, and device mismatch appears only as a benign per-synapse spread of the learning rate. On EMNIST letters with a 48-chip topology the forwards-only rule reaches $82.50\%$ against a full-backpropagation ceiling of $82.85\%$ on identical topology, and a bit-faithful hardware model stays within the run-to-run noise band of its floating-point ceiling. All results are pre-silicon.
author:
- Saul Dobney[^1]
bibliography:
- refs.bib
...
Two further principles are design commitments rather than hard invariants: the design is **modular** --- the network topology is a soft, reconfigurable property of the inter-chip router rather than fixed wiring --- and **hardware-feasible** --- every element is chosen to be synthesisable or fabricable in an open $130$ nm process (Sky130A). We began by testing the separated-weights architecture with contrastive Hebbian learning (CHL), the canonical predictive-coding rule, but found it too fragile under these constraints (Sec. 2); the rule reported here instead approximates the backpropagation gradient while respecting them.

The contributions follow the $\bm W\&\bm E$ organisation --- separate weight and error stores linked along a one-way signal path:

-   An architecture in which the chip is an autonomous predictive-coding *level*: it holds one copy of its weights $\bm W$ and reads that copy three ways --- forward prediction, transpose relay, and error accumulation --- so that no weight ever leaves the die and every weight update is a local transaction (Sec. 4).

-   *Transpose-at-source*: computing $\bm W^{\!\top}\bm\delta$ on the chip that owns $\bm W$, which eliminates the remote weight copy together with its coherence traffic and its synchronisation barrier, and reduces the router to summation and division (Secs. 4, 5).

-   A sigma--delta weight update in the error store $\bm E$ (the *leaky jug*), realised as a leaking capacitor and a shared comparator. The modulator is a standard structure; the contribution is its use as the weight-update path and the training-specific consequences of its bounded quantisation error --- why a learning rate far below one weight code step can drive a coarse analog cell with no per-synapse digital accumulator, why the comparator's decisions may be wrong at a substantial rate without corrupting learning, and why device mismatch acts only as a benign per-synapse learning-rate spread (Secs. 3, 4).
...
For readers coming from either side of this work, the recurring terms are collected here.

- **PC** --- predictive coding. **CHL** --- contrastive Hebbian learning, the canonical PC weight rule. **$\bm W\&\bm E$** --- the design's *separated* weight store $\bm W$ and error store $\bm E$.
- **$\bm\delta$** --- a transported error signal; **$\bm W^{\!\top}$** --- the transpose (column-wise read) of a weight matrix, needed to route error to the previous layer.
- **Weight code / quantisation step** --- the analog weight is set by an 8-bit digital code; one code step ($1/64$ of the weight range here) is the smallest weight change, the analog analogue of a *least-significant bit* (LSB).
- **Leaky jug** --- the per-synapse error accumulator: a capacitor that integrates error, leaks slowly, and "fires" one weight code step when it crosses a threshold. Its formal reading is a first-order **sigma--delta** ($\Sigma\Delta$) modulator.
- **Hardware acronyms** (Sec. 4 onward): **OTA**, operational transconductance amplifier (the analog multiplier cell); **MAC**, multiply--accumulate; **SRAM**, static memory holding the weight codes; **DAC/ADC**, digital-to-analog / analog-to-digital converter; **SAR**, successive-approximation register (the ADC type); **AGC**, automatic gain control; **RTL**, register-transfer level (a synthesisable digital description); **SPICE**, the analog circuit simulator; **NVM**, non-volatile memory.

# Predictive coding and the analog-neuromorphic context {#sec:pc}

## The $\bm W$ & $\bm E$ principle

Predictive coding models perception as inference in a hierarchical generative model: each level emits a prediction of the level below and retains the discrepancy as an error, and both representations and weights are adjusted to reduce it [@rao1999predictive; @bogacz2017tutorial]. Its defining structural commitment, and the one we build on, is that representation and error are carried by *separate* populations interacting only locally, one level apart. We refer to this as the $\bm W\&\bm E$ organisation: activations are propagated forward through the weights $\bm W$, errors are accumulated in a separate store $\bm E$, and when $\bm E$ crosses a threshold the corresponding weight in $\bm W$ is adjusted. Two properties make this a serious hardware substrate rather than a biological analogy. It is *locally computable* --- every quantity a synapse needs is present at that synapse, so an update can be performed in place rather than by a global gather --- and it has a *defensible relation to backpropagation*: a PC network with local Hebbian plasticity computes updates that converge to those of backpropagation when the output error is small relative to other activity [@whittington2017approximation]. We began our research by testing the $\bm W\&\bm E$ architecture with contrastive Hebbian learning, but the scheme we describe instead learns by approximating the backpropagation gradient, because of the difficulties --- set out next --- of making CHL work as a hardware solution.

## Four obstacles to an analog realisation of CHL {#sec:pc-challenges}

Canonical predictive coding using CHL, and the SNN tradition it is adjacent to, present four obstacles to an analog implementation. These are the reasons the search moved away from CHL; each is answered later by a specific element of the design.
...
Analog storage is sensitive to the size of each update. An effective learning rate for this task is $\eta \approx 3\times10^{-4}$, while the analog weight cell resolves $1/64$ per code. A single update is therefore $\approx 0.019$ of one weight code step: rounded to the cell's resolution it is exactly zero, roughly fifty times in succession. Writing a correct gradient to a coarse analog cell naively does not slow learning down; it stops it altogether. Updates can be passed through a high-precision digital *master weight* per synapse that accumulates sub-step updates and is periodically written down, but our hardware experiments suggested this requires $\sim 18$ bits of accumulator plus a velocity register per synapse --- a per-synapse digital memory larger than the analog weight it exists to serve.
...
The design answers C1 by relocating the transpose onto the chip that owns the weights $\bm W$ (Sec. 4), C2 by abandoning relaxation in favour of a single forward evaluation per sample (Sec. 3), C3 by replacing the digital master weight with a sigma--delta accumulator $\bm E$ (Sec. 3), and C4 by making the message class a configurable property of the interconnect (Sec. 5).

# The learning rule {#sec:theory}

This section states the rule as an algorithm, independently of how it is built; the hardware realisation follows in Sec. 4. The rule uses ordinary backpropagation's credit assignment, evaluated *once* per sample and written to the weights through a threshold-accumulator instead of an arithmetic add. The approach of backprop with the threshold-accumulator was chosen to meet the constraints of a hardware-first design. Core to this is the separation of weights and the forwards path, from the 'errors' and the error propagation path, with a leaky-accumulator used as the mechanism for updating the weights in the learning phase. The same credit assignment as ordinary backpropagation is computed, but it is delivered through a forwards-only, hardware-robust design --- a quantised threshold write over small-matrix networks rather than the massive matrix multiplies of conventional backpropagation.

Structurally the rule is a loop with two separated stores, and it is worth seeing the whole of it before the algebra. The forward path carries a sample through the weights, input $\to \bm W_1 \to \bm W_2 \to \dots \to$ read-out, and the read-out forms a single output error $\bm\delta$ --- one vector, not a field of them. That error is then relayed layer by layer, *backward through the network but forward in signalling*: each layer computes $\bm W^{\!\top}\bm\delta$ from the weights it already holds, hands the result to the layer before it, and drops its own share into a local error store. The store is a leaky accumulator --- a *jug* --- that integrates what it receives and, only when it crosses a threshold, writes a single $\pm1$ code step into that layer's weights. There is no relaxation phase and no reverse analog pass: one forward sweep, one error relay, one conditional weight step per synapse (Fig. 0).
...
<!-- FIGURE 0 (fig:overview) — TO BE DRAWN in the .tex (TikZ). High-level dataflow:
     ...
     Caption comes from the build script; edit there for changes. -->
...
Write $\bm x_0$ for the input feature vector and, for layers $\ell = 1,\dots,L$, $$\bm a_\ell = \bm W_\ell \bm x_{\ell-1}, \qquad \bm x_\ell = \varphi(\bm a_\ell),$$ with $\varphi$ a leaky rectifier ($\varphi(u) = u$ for $u \ge 0$, $\alpha u$ otherwise, $\alpha = 0.1$). Each $\bm W_\ell$ is block-structured by the chip factoring of Sec. 4: a chip owns a set of $16\times16$ blocks and no block spans chips. A digital read-out $\bm s = \bm W_f \bm x_L + \bm b_f$ produces class scores, and the output error for label $y$ is $$\bm e = \mathrm{onehot}(y) - \hat{\bm p},$$ $\hat{\bm p}$ being the read-out's class estimate. One further quantity is read from the same scores: a scalar *happiness* $h$, the read-out's margin quantised to a few levels ($0$--$7$), large only when a sample is misclassified or is correct by a thin margin and zero when it is comfortably correct. It carries no per-unit information --- one number per sample --- and is broadcast to scale the update severity (Sec. 3.3); pinning it to a constant recovers a fixed learning rate. The error is then transported by the standard recursion, evaluated *once* per sample --- there is no relaxation phase, which is how C2 is discharged: $$\begin{aligned}
...
\end{aligned}$$ Equation (4) is exactly backpropagation's recursion; the architectural content, developed in Sec. 4, is *where* each factor is evaluated. The product $\bm W_\ell^{\!\top}\bm\delta_\ell$ is a transpose of the layer's own weights; the Jacobian factor $\varphi'(\bm a_{\ell-1})$ is a sign test on that layer's forward activation; and the sum over the parts contributing to a destination is a plain addition. No unit needs a weight it does not own.

Two operators appear, and both are consequences of quantised, finite-range signalling rather than of the mathematics. $\mathcal{Q}$ is a fixed-range $b$-bit quantiser ($b = 6$ here) --- the finite resolution of the error path. $\mathcal{A}_\ell$ is a slow per-layer automatic gain control (AGC) that tracks the running root-mean-square (RMS) of the layer's error with an exponential moving average and rescales it to a fixed target. The AGC is not cosmetic. The transported error attenuates by $\approx 0.72\times$ per layer --- the classical vanishing-gradient factor [@bengio1994learning], set by the weight norm --- so that under a *fixed-range* quantiser the error eventually underflows the smallest code and $\mathcal{Q}$ maps it identically to zero, at which point the layer stops learning. Measured usable depth is 4 layers without AGC and beyond 12 with it; the AGC is what makes the quantised error path deep-scalable, and a 6-bit path with AGC outperforms a 10-bit path without.
...
The alternative is to buy resolution with *time*. The error store is a value $E$ that is never reset. It leaks; incoming error tops it up; and when it crosses a threshold $\theta$ the synapse *fires*: the weight code moves by one step and $\theta$ is *subtracted* from the store, retaining the residue (Fig. 3). For one synapse, per sweep $t$, $$E_t = \lambda E_{t-1} + g_t, \qquad
...
<!-- FIGURE 3 (fig:jug) — drawn in TikZ in main_stage2_v3.tex; not representable in Markdown.
     ...
     Caption comes from v3.tex; this blockquote is a placeholder for the .md reader only. -->

> **Figure 3 (fig:jug).** **The leaky jug.** Error accumulates in the store; it leaks, which supplies momentum; a threshold crossing fires one weight code step and subtracts $\theta$, retaining the residue. The retained residue is what makes the mechanism a sigma--delta modulator rather than a lossy quantiser.


The structure is a first-order sigma--delta (delta--sigma) modulator, standard in data conversion and previously used to code activations in neural networks [@oconnor2017sigma]. What is specific to this work is its *role* --- it is the weight-update path, which is what lets a coarse analog cell learn with no per-synapse accumulator --- and the training-specific corollaries the bounded-error property yields; the modulator itself is not new.

#### Bounded, non-accumulating quantisation error.

Take $\lambda = 1$ and sum (5) over $T$ sweeps. Whatever the comparison decides, the update to $E$ and the update to $W$ are the *same* event, so the identity $$\theta \sum_{t=1}^{T} s_t \;=\; \sum_{t=1}^{T} g_t \;-\; E_T
\label{eq:invariant}$$ holds exactly. The left side is $\theta$ times the total motion of the weight code; the first term on the right is the total accumulated gradient. Since the subtraction removes exactly $\theta$ whenever the level reaches $\theta$, and the injection per sweep is small, $|E_T|$ remains bounded by the store's clamp $E_{\max}$. Therefore $$\Bigl| \sum_{t=1}^{T} s_t - \frac{1}{\theta}\sum_{t=1}^{T} g_t \Bigr|
\;\le\; \frac{E_{\max}}{\theta},
\label{eq:bound}$$ where $\sum_t s_t$ is the total motion of the weight code --- a bound *independent of $T$*. The weight tracks the accumulated gradient scaled by $1/\theta$, with an error that does not grow with the length of training. This is the sigma--delta property, and it is the mathematical content of the design: rounding each update discards a fixed fraction of every step (here, all of it); the jug defers steps but discards none.

Three corollaries follow directly, and each is confirmed in Sec. 6.

*$\theta$ is the learning rate.* By (7) the weight moves $\sum_t g_t/\theta$ codes, so $\theta$ sets the effective step and is the parameter to tune. It is a fixed reference, set once, and is never discovered at runtime.

*The comparison may be wrong.* Identity (6) does not assume $s_t$ is correct. A wrong-signed fire moves the weight the wrong way *and* pushes the residue further from zero by $\theta$, which makes the next fire more likely and of the correct sign; the accumulated quantity is conserved and the error is repaid. What must be accurate is the *stored quantity*, not any decision made about it. This inverts the usual analog design burden: the threshold comparison, normally a precision operation, becomes the loose one.
...
\lambda^{t-k} g_k$, which is the exponentially-weighted velocity of a momentum optimiser. The store's leak and the optimiser's velocity register are the same object, so the register is not implemented but inherited. In the hardware (Sec. 4) the physical leak available is far slower than needed, so on the firing timescale the store behaves as a pure integrator, which is the best-performing configuration measured; the requirement is a loose one-sided bound, not a servo.

# Hardware realisation {#sec:arch}

Having fixed the rule, we now describe how each of its computations is carried out in silicon. The design is a mixed analog--digital one that allows flexible topologies: the core compute is analog, done with capacitance and transconductance, but the weight *values* are held digitally in SRAM to protect them from decay on a capacitor. We describe it from the unit that matters most --- the chip and its single weight memory --- outward to the network.

The commitments a PCN or SNN reader can hold throughout are these. (i) Representation and error are separate populations ($\bm W\&\bm E$). (ii) Each chip keeps exactly one copy of its weights in SRAM and reads it three ways --- forward prediction, transpose relay, and error accumulation. (iii) The inter-chip router only *gathers* (sum and divide); it holds no weights, so the network topology is a soft, reconfigurable property of the router rather than fixed wiring. (iv) All signalling is forward-directed; there is no reverse analog channel and no counter-propagating pipeline. (v) The update approximates the backpropagation gradient of Sec. 3, driven by the minimal "happiness" scalar that gates severity without carrying per-unit information. (vi) Every element is chosen to be synthesisable or fabricable in an open $130$ nm process (Sky130A).

## The substrate and its granularity

The compute element, inherited from Part I [@dobney2026analog], is a five-transistor operational transconductance amplifier (OTA): an 8-bit code held in SRAM (one step per $1/64$ of the weight range) is converted to a voltage on a $200$ fF capacitor, which sets the tail current of a differential pair and hence the transconductance by which the cell multiplies its input. Column currents sum on a shared node by Kirchhoff's current law --- the analog multiply--accumulate (MAC) --- and an 8-bit successive-approximation-register (SAR) analog-to-digital converter (ADC) digitises the result. The array unit is a $16\times16$ *cell* ($256$ such multipliers); a *chip* carries four cells; a *router* mediates all traffic between chips. Routing is performed at cell granularity and never finer, which keeps the routing state $O(\text{cells}^2)$ rather than $O(\text{synapses}^2)$; fine-grained mixing is the job of the destination cell's own weight matrix, not of the network. Because the router, not the wiring, defines which cell feeds which, the topology is reconfigurable and open to experiment.

#### The weight cell is non-linear, and is linearised by pre-distortion.

The five-transistor cell's effective weight --- the differential-pair transconductance --- is not linear in the stored code: it peaks near mid-range and falls above it, so the upper half of the nominal range is compressed and, at the extreme, inverted. This is a property of the cell, measured across the full code range in SPICE (a single-operating-point check cannot see it). The design linearises it in two steps: the tail device is sized to restore monotonicity, and the weight digital-to-analog converter (DAC) is driven through a fixed pre-distortion table (Fig. 1) so that stored code maps monotonically to effective weight. With the measured cell curve and this pre-distortion in place, accuracy is $81.46\%$ --- within the run-to-run noise band of the $81.96\%$ obtained under an ideal linear cell (Sec. 6) --- so the bit-faithful model's linear-weight treatment is a consequence of the pre-distortion, not an assumption.
...
1.  **Forward prediction ($\bm W$).** The code is mapped through a pre-distortion look-up table to the weight DAC and the analog array multiplies the incoming activation. This is the in-memory forward pass and realises $\bm a_\ell = \bm W_\ell \bm x_{\ell-1}$.
...
3.  **Error accumulation ($\bm E$).** The incoming error, scaled by the locally latched forward activity, forms the per-synapse increment $g_t$ of (5); a threshold crossing writes a single $\pm1$ step back to the SRAM.
...
## The error store in hardware {#sec:jughw}

The rule's update path (5)--(7) is realised by a leaking capacitor $C_e$ and a shared comparator. The mathematics asked for three things of this circuit, and each turns a normally-tight analog requirement into a loose one.

*One tight specification, and only one.* The mechanism asks for analog accuracy in exactly one place: the residue subtraction must remove a fixed quantity $\theta$ independent of the capacitor's voltage, realised as a current-steered charge pulse $Q = It$. Identity (6) is the reason it is the *only* tight specification --- because the stored quantity is conserved across the fire event, everything downstream of it, the comparator decision included, is permitted to be loose.

*The comparator is shared and swept, not per-synapse.* A single comparator walks the array, so a synapse is tested once per sweep and can fire at most once per sweep. This is a design commitment rather than an approximation: it is $256\times$ cheaper per cell, and the sweep period acts as a refractory interval. A per-synapse comparator would allow repeated firing on a single crossing; we measure this to be unstable (Sec. 6). A synapse holding $3\theta$ therefore emits a train of three single fires on successive sweeps rather than one triple fire --- the boost is delayed, never discarded. Clamping the capacitor harder would be the wrong rate limiter, as it destroys the stored quantity that (6) requires.

*The leak physically realises momentum.* The available leak is $\approx 50\,\mu$V/s on $100$ fF, i.e. $\approx 140$ s per error code step, against a mean inter-fire interval of tens of milliseconds --- so on the firing timescale the capacitor is a pure integrator, the momentum limit $\lambda \to 1$ of Sec. 3, and the requirement $\tau_{\text{leak}}\gg$ inter-fire interval is met by a factor of $10^3$.

*The error is kept out of the forward path.* $C_e$ is compared, never summed into the MAC. An earlier cell placed a second tail transistor driven by $C_e$ so that the array computed $\bm W+ \bm E$. That coupling is a positive feedback loop --- error inflates activations, which inflate errors --- and Sec. 6 shows it degrades accuracy at the old operating point and collapses the network beyond it. Removing it returns the cell to the original single-tail topology and dissolves a biasing constraint, so the simulator and the silicon compute the same function.
...
# Separability, modularity, and scale {#sec:scale}

One decision underlies every property claimed above, and it is clearest stated here, once the pieces are on the table. The decision is to **separate the weights from the errors** --- to treat the forward computation and the learning signal as two stores on two paths ($\bm W\&\bm E$), rather than as one object updated in place by a backward pass coupled to the forward one. The three invariants of the introduction, and the modularity that lets the fabric grow, are consequences of that separation rather than independent achievements.

**Separation makes the neuron self-contained.** Once the error travels as an ordinary forward-directed message rather than as a current pushed back through the array, a weight is never needed anywhere but on the die that owns it. That is what allows a single weight memory to serve all three of its consumers --- forward prediction, transpose relay, and error accumulation --- from one copy. There is no master weight held elsewhere, no second copy to keep coherent, and no quantity in a weight update that lives off the die.

**Self-containment makes the tile modular.** A unit that needs nothing external can be replicated without qualification. The analog array, its transpose engine, and its jug together form a complete learning cell; a network is assembled by tiling copies of that cell and wiring them through the router, not by designing a fresh datapath for each layer or width. The cell that learns one layer learns any layer, which is why the same $16\times16$ tile is the unit of both computation and growth.

**Modularity makes the system scalable.** Because every tile updates locally and asynchronously --- each has on hand every quantity its update needs, and no step waits on a global phase --- enlarging the network is a matter of adding tiles and interconnect, not of re-establishing coherence across a larger fabric. The property that makes large analog neuromorphic systems fragile, a global schedule holding forward and backward passes in lockstep, is absent by construction. What bounds scale is interconnect bandwidth and area, both of which grow gracefully, rather than a synchronisation cost that worsens with size.

Read forwards, the chain is *separation* $\to$ *self-containment* $\to$ *modularity* $\to$ *scale*, and it is why the invariants cost so little accuracy rather than so much. Forwards-only is what permits the single weight copy; robustness is what lets a coarse, mismatch-ridden cell be replicated without per-die calibration; asynchrony is what lets the replicas run without a shared clock. They are not three separate constraints that happened to be satisfiable together, but three aspects of the one separation --- and the accuracy figures of the evaluation are the measurement that paying for it costs essentially nothing.
