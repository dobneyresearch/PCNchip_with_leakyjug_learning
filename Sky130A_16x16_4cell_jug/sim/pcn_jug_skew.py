#!/usr/bin/env python3
"""pcn_jug_skew.py — pcn_jug.py + the FORWARD↔TRANSPOSE WEIGHT-SKEW TEST.

★ A COPY of pcn_jug.py (do NOT edit the validated sim). The ONLY additions are `--skew_folds`
  and `--skew_mode`, which make the backward TRANSPOSE read a version of W that differs from the
  W the forward used — i.e. it models the temporal weight-skew of the async, transpose-at-source
  design (weights move between the forward and the moment the δ comes back to be transposed).

WHY: under transpose-at-source the chip's forward and backward both read the LIVE W-SRAM. The
sweep-barrier (fires happen only at the end-of-fold sweep, which by data-dependency can't run
until the fold's backward is done) means forward & backward see the SAME W by construction —
skew ≈ 0. This test CONFIRMS the baseline (skew_folds=0 must reproduce the headline) and BOUNDS
the pipeline-overlap VIOLATION (skew_folds>0 = the transpose reads W offset by k folds of real
fire activity).

  --skew_folds 0     the invariant HOLDS (current sim). Must reproduce ~81.96%.
  --skew_folds k     transpose reads W as it was k folds ago (lag) OR W±random-drift (rand).
  --skew_mode lag    faithful: the SAME fires, time-shifted by k folds (correlated drift).
  --skew_mode rand   a cruder independent bound: ±1 code on ~2%·k of synapses, fresh each fold.

Only the two W-TRANSPOSE reads are skewed (L3→L2 and L2→L1). The forward, the E-accumulation
(outer products), and the sweep all use the LIVE W — exactly as the hardware does.

Run:   python3 pcn_jug_skew.py --epochs 4 --chunk 8000 --skew_folds 0   (baseline)
       python3 pcn_jug_skew.py --epochs 4 --chunk 8000 --skew_folds 4   (bound the violation)
=============================================================================================
...
def main():
    ...
    p.add_argument('--skew_folds', type=int, default=0,
                   help='TRANSPOSE-AT-SOURCE async test. The backward W.T reads a W that differs '
                        'from the forward W by k folds of drift. 0 = the sweep-barrier invariant '
                        'holds (must reproduce the headline). k>0 bounds the pipeline-overlap '
                        'violation (weights moved between forward and the δ coming back).')
    p.add_argument('--skew_mode', default='lag', choices=['lag', 'rand'],
                   help="lag: transpose reads W as it was skew_folds folds ago (faithful — the "
                        "SAME fires, time-shifted, correlated drift). rand: ±1 code on ~2%%*k of "
                        "synapses, fresh each fold (a cruder, independent, uncorrelated bound).")
    a = p.parse_args()
    ...
    # ── FORWARD↔TRANSPOSE WEIGHT-SKEW machinery ────────────────────────────────────────────
    # skew_buf holds deep snapshots of Wm; the transpose reads the OLDEST (skew_folds folds ago).
    # Persists across epochs (drift is continuous). At skew_folds=0, Wm_bwd IS Wm (no skew).
    skew_buf = deque(maxlen=max(1, a.skew_folds))

    def perturb_w(W):
        """rand mode: ±1 code on ~ (2% * skew_folds) of synapses, clipped to the rails. A fresh,
        UNCORRELATED perturbation of the same magnitude as skew_folds folds of ~2%/fold firing."""
        frac = min(1.0, 0.02 * a.skew_folds)
        out = copy.deepcopy(W)
        for L in range(3):
            for c in range(len(out[L])):
                for pg in range(len(out[L][c])):
                    w = out[L][c][pg]
                    m = hw.rng.random(w.shape) < frac
                    st = np.where(hw.rng.random(w.shape) < 0.5, -STEP, STEP)
                    w[m] = np.clip(w[m] + st[m], WMIN, WMAX)
        return out

    if a.skew_folds > 0:
        print(f"  ⚠ WEIGHT-SKEW TEST: transpose reads W offset by {a.skew_folds} folds "
              f"(mode={a.skew_mode}). skew_folds=0 is the invariant-holds baseline.", flush=True)
    ...
    for ep in range(1, a.epochs + 1):
        ...
        for b0 in range(0, a.chunk - a.fold_every + 1, a.fold_every):
            ...
            # ── select the weights the TRANSPOSE will read (may be skewed from the forward) ──
            if a.skew_folds == 0:
                Wm_bwd = Wm                                   # invariant holds: same live W
            elif a.skew_mode == 'lag':
                # the oldest snapshot = skew_folds folds ago (once warmed up); else no skew yet
                Wm_bwd = skew_buf[0] if len(skew_buf) == a.skew_folds else Wm
            else:                                             # rand
                Wm_bwd = perturb_w(Wm)
            ...
            for g in range(B.N_L3_CHIPS):
                Dg = D3[:, g*N_ROWS:(g+1)*N_ROWS]
                for p, li in enumerate(B.L3_ROUTING[g]):
                    # THE TRANSPOSE W.T — reads Wm_bwd (= Wm unless --skew_folds skews it).
                    Ws = hw.wgt(Wm_bwd[2][g][p], 'bwd')
            ...
            for g in range(B.N_L2_CHIPS):
                ...
                st = g * B.N_L1_PER_L2 * N_ROWS
                for p in range(B.N_PAGES):
                    Ws = hw.wgt(Wm_bwd[1][g][p], 'bwd')       # skewed transpose read
                    c = g * B.N_L1_PER_L2 + p
            ...
            # ── record this fold's post-sweep W for the LAGGED transpose read ──────────────
            if a.skew_folds > 0 and a.skew_mode == 'lag':
                skew_buf.append(copy.deepcopy(Wm))
